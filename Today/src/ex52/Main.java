...
public class Main {
	...
	public static void main(String[] args) {
		...
		result = sol.solution(new String[] {"cafelatte", "americanoice", "hotcafelatte", "anything"}); 
		System.out.println("19000=="+result); 
		
		result = sol.solution(new String[] {"americanoice", "americano", "iceamericano"}); 
		System.out.println("13500=="+result); 
	}
}

class Solution {
    public int solution(String[] order) { // 0.03ms ~ 1.7ms
        int answer = 0;
        
        for (int i = 0; i < order.length; i++) {
			if(order[i].contains("ame") || order[i].contains("any")) {
				answer += 4500;
			}else {
				answer += 5000;
			}
